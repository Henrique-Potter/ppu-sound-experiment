...
def generate_effects():
    ...
    for phaser in np.arange(0.1, 1.1, 0.1):
        fx = (
            AudioEffectsChain().pitch(phaser)
        )
        fxs.append(['Phaser {}'.format(phaser), fx])
    ...
    for fx in fxs:
        ...
        for id, same_person_audio_list in data_set.items():
            for audio_info in same_person_audio_list:
                ...
                raw_audio_s2t, nframes, frame_rate = ml_engine.load_audio_file(raw_wav_path)
